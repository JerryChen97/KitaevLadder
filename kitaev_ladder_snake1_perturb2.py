...
class KitaevLadderSnakeCompactPerturb(Lattice):    
...
class KitaevLadderSnakeCompactPerturbModel(CouplingMPOModel):
    ...
    def init_lattice(self, model_params):
        ...
        lat = KitaevLadderSnakeCompactPerturb(L, gs, **lattice_params)
        return lat
    ...
    def init_terms(self, model_params):
        ...
        if S==0.5:
            for u1, u2, dx in self.lat.pairs['nearest_neighbors_x']:
                ...
                self.add_coupling(k, u1, 'Sigmaz', u2, 'Sigmaz', dx)
            ...
            for u1, u2, dx in self.lat.pairs['nearest_neighbors_z']:
                self.add_coupling(Jz, u1, 'Sigmaz', u2, 'Sigmaz', dx)# This is the perturbation for breaking Z
                
            # for u in range(len(self.lat.unit_cell)): # This is the perturbation for breaking XY
            ...
            for u1, u2, dx in self.lat.pairs['nearest_neighbors_x']:
                ...
                self.add_coupling(k, u1, 'Sz', u2, 'Sz', dx)
            for u1, u2, dx in self.lat.pairs['nearest_neighbors_y']:
                self.add_coupling(Jy, u1, 'Sy', u2, 'Sy', dx)
                self.add_coupling(k, u1, 'Sz', u2, 'Sz', dx)         
            for u1, u2, dx in self.lat.pairs['nearest_neighbors_z']:
                self.add_coupling(Jz, u1, 'Sz', u2, 'Sz', dx)# This is the perturbation for breaking Z

            # for u in range(len(self.lat.unit_cell)): # This is the perturbation for breaking XY
            #     self.add_onsite(g, u, 'Sz')
        
def plot_lattice():
    fig, ax = plt.subplots()
    lat = KitaevLadderSnakeCompactPerturb(5, None, bc='open')
...
def run_atomic(
    ...
    M = KitaevLadderSnakeCompactPerturbModel(model_params)
...
def naming(
    ...
    return "KitaevLadderPerturb"+"_chi_"+str(chi)+"_Jx_"+str(Jx)+"_Jy_"+str(Jy)+"_Jz_"+str(Jz)+"_k_"+str(k)+"_L_"+str(L)
...
def full_path(
    ...
    **kwargs):
    return prefix+naming(chi, Jx, Jy, Jz, k, L)+suffix
...
def load_data(
    ...
    file_name = full_path(chi, Jx, Jy, Jz, k, L, prefix=prefix, suffix='.h5')
